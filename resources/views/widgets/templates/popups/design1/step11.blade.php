<div style="display: none;" class="display_div d1 step_11" id="ar_form">
        @if( (isset($widget) && isset($widget->phone_field_active) && $widget->phone_field_active == 1) || (isset($widget) && isset($widget->email_field_active) && $widget->email_field_active == 1) ||(isset($widget) && !empty($widget->rawhtml_form_action)) || (isset($widget) && !empty($widget->email_provider) && !empty($widget->email_provider_value)) )
            @if( !(isset($widget) && !empty($widget->rawhtml_form_action) ) && !( isset($widget) && !empty($widget->email_provider) && !empty($widget->email_provider_value)) )
                <form method="post" class="video_form video_form_phone video_form_one" id="video_form" enctype="multipart/form-data">
            @elseif( isset($widget) && !empty($widget->email_provider) && !empty($widget->email_provider_value) )
                <form action="{!! url('settings/create-subscriber',[$widget->user_id, $widget->email_provider, $widget->email_provider_value]) !!}" method="post" class="video_form video_form_two" >
            @else
                <form action="{!! url('settings/create-row-subscriber') !!}" method="post" class="video_form raw_video_form video_form_three" enctype="multipart/form-data">
            ...
            @endif
        @else
            <form method="post" class="video_form_four">
        @endif

        <div id="popup11" class="lightbox video_subscribe_popup" @if(isset($widget)) style="background-color: {!! $widget->widget_bg_color !!}" @endif>
                    ...
                    <fieldset>
                        ...
                        <div class="btn-holder">
                            {{--@if((isset($widget) && isset($widget->phone_field_active) && $widget->phone_field_active == 0) && ( (isset($widget) && !empty($widget->rawhtml_form_action)) || (isset($widget) && !empty($widget->email_provider) && !empty($widget->email_provider_value)) ))--}}
                                {{--<button type="submit" class="btn-send" id="validate_inputs_video" @if(isset($widget)) style="background: {!! $widget->widget_buttons_bg_color !!} ; color: {!! $widget->widget_buttons_text_color !!} !important;" @endif><span>Send</span></button>--}}
                            {{--@elseif((isset($widget) && isset($widget->phone_field_active) && $widget->phone_field_active == 1) && (isset($widget->country_code) && $widget->country_code == "US"))--}}
                                {{--<button type="button" class="btn-send video-send-phone" id="validate_inputs_video" @if(isset($widget)) style="background: {!! $widget->widget_buttons_bg_color !!} ; color: {!! $widget->widget_buttons_text_color !!} !important;" @endif><span>Send</span></button>--}}
                            {{--@else--}}
                                {{--<a href="javascript:void(0)" class="btn-send dsgn1s11 send_video_message" id="send-video" @if(isset($widget)) style="background-color: {!! $widget->widget_buttons_bg_color !!} ; color: {!! $widget->widget_buttons_text_color !!} !important;" @endif>Send</a>--}}
                            {{--@endif--}}
                            <a href="javascript:void(0)" class="btn-send dsgn1s11 send_video_message" id="send-video" @if(isset($widget)) style="background-color: {!! $widget->widget_buttons_bg_color !!} ; color: {!! $widget->widget_buttons_text_color !!} !important;" @endif>Send</a>
        ...
        (function($) {
            $( document ).ready(function() {
                    ...
                    $('.send_video_message').on('click', function() {
                        ...
                        $.post( $(this).closest('form').attr('action') , dataStr);

                        if($('.video_form').length !=0){

                            if($('#ar_form').is(":visible")){
                                $('#ar_form').hide();
                                console.log('d1 s11 L162');
                                $('.display_div').hide();
                                console.log('d1 s11 L164');
                                $('.thankyou-page').show();
                                console.log('d1 s11 L166');
                            } else {
                               $('.display_div').hide();
                               console.log('d1 s11 L169');
                               $('#ar_form').show();
                               console.log('d1 s11 L171');
                            }
