<div style="display: none;" class="display_div d2">
    <div id="popup21" class="lightbox" @if(isset($widget)) style="background-color: {!! $widget->widget_bg_color !!}" @endif>
        <section class="popup popup-five popup21">
            ...
            <strong class="title" @if(isset($widget)) style="color: {!! $widget->widget_text_color !!}" @endif>Press “Play” to listen to your recording Press “Re-record” to record a new message Press “Send” to send your voice message</strong>
            <div class="border" @if(isset($widget)) style="border-color: {!! $widget->widget_buttons_bg_color !!}" @endif>
                <h3 @if(isset($widget)) style="color: {!! $widget->widget_text_color !!}" @endif><span id="spanimg" style="display: none; margin-left: 10px;"><b @if(isset($widget)) style="color: {!! $widget->widget_text_color !!}" @endif>Processing </b><img id="loader-img" alt="" src="/assets/img/loader.gif" width="20" height="20" align="center" /></span><b id="playing" style="display: none;">Playing <span id="message_length" @if(isset($widget)) style="color: {!! $widget->widget_buttons_bg_color !!} !important;" @endif></span></b></h3>
                ...
                <div class="btn-holder hidden_div" style="display: none !important;">
                    <a id="pButton" class="btn-play" onclick="play()" @if(isset($widget)) style="background-color: {!! $widget->widget_buttons_bg_color !!} ; color: {!! $widget->widget_buttons_text_color !!} !important;" @endif>Play</a>
                    <a href="javascript:void(0)" class="btn-stop" id="re-record">Re-record</a>
                    @if((isset($widget->first_name_field_active) && $widget->first_name_field_active == '1') || (isset($widget->email_field_active) && $widget->email_field_active == '1') || (isset($widget->phone_field_active) && $widget->phone_field_active == '1'))
                        <a class="btn-send" @if(isset($widget)) style="background-color: {!! $widget->widget_buttons_bg_color !!} ; color: {!! $widget->widget_buttons_text_color !!} !important;" @endif>Next</a>
                    @else
                        <form method="post" enctype="multipart/form-data" id="voice_form">
                            ...
                            <a id="send-button" class="btn-send send_voice_message" @if(isset($widget)) style="background-color: {!! $widget->widget_buttons_bg_color !!} ; color: {!! $widget->widget_buttons_text_color !!} !important;" @endif>Send</a>
                        </form>
                    @endif
                </div>
