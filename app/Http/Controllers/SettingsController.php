...
class SettingsController extends Controller {
	...
	public function index( EmailServicesServiceInterface $emailService, SmsServicesServiceInterface $smsService, CampaignServiceInterface $campaignService )
	{
		...
		return view('settings.index' , [ 'emailSettings' => $emailService->getAllEmailServices(), 'smsSettings' => $smsService->getAllSmsServices(), 'campaigns' => $campaignService->getAllCampaigns(), 'youtube_connected' => $youtube_connected ]);
	}
	...
	private function updateMessage($inputs)
	{
		...
		if(null != $message = $this->messageService->updateMessages( $inputs['id'], $this->request->inputs($campaign_id, $widget_id, $user_id , $name , $inputs['email']) )) {
			...
			if(count($this->youtubeService->isConnected($user_id))){
				...
				// } else {
					try {
						$youtube_id = $this->youtubeService->uploadVideoToYoutube($this->messageService->getMessageById($id)->file_name);
			...
			if((isset($widget->map_field_1_active) && $widget->map_field_1_active == '1') || (isset($widget->map_field_2_active) && $widget->map_field_2_active == '1') || (isset($widget->map_field_3_active) && $widget->map_field_3_active == '1'))
			{
				$this->widget_optin_service->addWidgetOptin( $widget->id );
			}
			$messageInfo = $this->messageService->getMessageById($inputs['id']);
			$this->messageService->sendEmailWithSandLane( $email , $url , $messageInfo );
...
//		return response()->json(['form_action' => $form_action, 'token' => $token, 'widget' => $widget, 'inputs' => $inputs]);
